...
def map_finish_to_stop_reason(
    ...
    # Claude Code treats stop_reason=tool_use with zero tool_use content blocks
    # as a malformed tool call ("The model's tool call could not be parsed") and
    # retries then aborts. Only report tool_use when a tool_use block was actually
    # emitted — never solely because upstream finish_reason was tool_calls.
    if has_tool_calls:
        return "tool_use"
...
def _effective_tool_arguments_json(
    args: Any, *, tool_name: str | None = None
) -> str:
    """Normalize tool arguments; fill "{}" for empty zero-required tools.

    Models / relays often omit ``function.arguments`` entirely (or send "") for
    schema-less tools such as EnterPlanMode / TaskList / ExitPlanMode /
    CronList. Claude Code still needs a tool_use block with ``input: {}`` —
    otherwise it sees ``stop_reason=tool_use`` with zero tool_use blocks and
    aborts with "The model's tool call could not be parsed (retry also failed)".
    """
    text = normalize_tool_arguments_json(
        args if args is not None else "", tool_name=tool_name
    )
    text = (text or "").strip()
    if text:
        return text
    # Empty / missing → only invent "{}" when the tool has no required keys
    # (or the tool name is unknown free-form). Required-key tools stay empty
    # so incomplete previews are not force-emitted.
...
def is_complete_tool_arguments_json(
    ...
    Bare `{}` / `[]` are ready only for tools with no required keys (e.g.
    EnterPlanMode, TaskList, ExitPlanMode). For tools that require fields,
    empty placeholders stay incomplete so Grok previews cannot open a ghost
    block before real arguments arrive.
    ...
    if not isinstance(parsed, (dict, list)):
        return False
    required = _required_keys_for_tool(tool_name) if isinstance(parsed, dict) else ()
    # Empty object/array is a valid final payload for zero-required tools
    # (EnterPlanMode / TaskList / CronList / ...). For required-key tools,
    # hold empty placeholders until real fields arrive.
    if parsed == {} or parsed == []:
        return not required
...
def openai_tool_calls_to_content_blocks(
    ...
    *,
    ...
    Incomplete / unparseable tool arguments are dropped rather than shipped as
    ``{"_raw": "..."}`` — Claude Code treats unparsed input as a malformed tool
    call. Empty-arg tools with no required schema keys become ``input: {}``.
    """
    ...
    for tc in tool_calls:
        ...
        args_text = _effective_tool_arguments_json(args_raw, tool_name=name)
        if not args_text or not is_complete_tool_arguments_json(
            args_text, tool_name=name
        ):
            # Incomplete required-key tool (or garbage) — skip rather than emit
            # a broken tool_use that Claude Code will reject/retry-fail.
            continue
        parsed = _parse_tool_arguments(args_text)
        # Never ship unparsed residual blobs to Claude Code.
        if isinstance(parsed, dict) and set(parsed.keys()) == {"_raw"}:
            continue
        if not name:
            continue
        blocks.append(
...
class AnthropicStreamAssembler:
    ...
    def finish(
        ...
        *,
        ...
        # Fill "{}" for empty zero-required tools before readiness checks so
        # EnterPlanMode / TaskList / … are treated as ready at finish even when
        # the model omitted function.arguments entirely.
        for _s in self._tools.values():
            if _s.get("stopped") or _s.get("started"):
                continue
            _name = (_s.get("name") or "").strip()
            if not _name:
                continue
            _eff = _effective_tool_arguments_json(
                _s.get("args") or "", tool_name=_name
            )
            if _eff and _eff != (_s.get("args") or "").strip():
                _s["args"] = _eff
